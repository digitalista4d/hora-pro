...
function adicionarProduto() {
  if (totalProdutos >= limiteGratis) {
    document.getElementById('limiteAviso').innerText =
      'Limite de 5 produtos atingido no plano gratuito. Atualize para o Plano Pro ou Ultra para continuar adicionando.';
    return;
  }
  ...
  const novaLinha = `<tr><td>${produto}</td><td>${custo}</td><td>${venda}</td><td>${qtd}</td><td><strong>${lucro} Kz</strong></td></tr>`;
  document.getElementById('tabelaInventario').insertAdjacentHTML('beforeend', novaLinha);

  historicoExtrato.push(
    `Produto: ${produto}, Custo: ${custo}, Venda: ${venda}, Qtd: ${qtd}, Lucro: ${lucro} Kz`
  );
